 ...
 * @author 1U02UN
 */
public class Alter_s extends javax.swing.JFrame {
    ...
    private void jButton_alterActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton_alterActionPerformed
        // TODO add your handling code here:
        //如果输入的内容为空，则弹窗警告输入内容！不为空则判断是否有SQL注入，有就弹窗警告重新输入！        
        if(checkSupplierInput()){
            ...
            String sql = "update supplier set name='"+name+"',phone='"+phone+"',email='"+mail+"',address='"+address+"' where id="+id;
            DBHelper.update(sql);
            javax.swing.JOptionPane.showMessageDialog(this, "供应商信息修改成功！", "提示", javax.swing.JOptionPane.INFORMATION_MESSAGE);			                
            dispose();    
            try {
    ...
    //对输入内容进行检查，检查通过则返回为真    
    public boolean checkSupplierInput(){
        if(jTextField_name.getText().isEmpty()){
           ...
           showMessageDialog(this, "供应商电话不能为空！", "警告",  javax.swing.JOptionPane.WARNING_MESSAGE);
           ...
           jTextField_address.requestFocus();
               ...
               showMessageDialog(this, "供应商包含SQL注入敏感字符，请重新输入！", "警告",  javax.swing.JOptionPane.WARNING_MESSAGE);
               jTextField_name.requestFocus();
        }else if(!filter.isPhone(jTextField_phone.getText())){
               showMessageDialog(this, "供应商电话格式输入错误，请输入3-11位数字！", "警告",  javax.swing.JOptionPane.WARNING_MESSAGE);          
               jTextField_phone.requestFocus();
    ...
    public void getSupplierInfo(){
        String name=jTable_Supplier.getValueAt(jTable_Supplier.getSelectedRow(),1).toString();
        String phone=jTable_Supplier.getValueAt(jTable_Supplier.getSelectedRow(),2).toString();
        String mail=jTable_Supplier.getValueAt(jTable_Supplier.getSelectedRow(),3).toString();
        String address=jTable_Supplier.getValueAt(jTable_Supplier.getSelectedRow(),4).toString();
